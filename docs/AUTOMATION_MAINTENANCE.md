...
| NotebookLM reading notebook | Manual source import on version change | Planned weekly review of listed GitHub source references and stale snapshots; not active because the scheduled-task limit is reached | Google Notebook currently has no connector here; notebook source updates cannot be promised as automatic |
| Lovable Research Compass | Weekly connected-app review | Planned connected-app check of build state, publication state and connector configuration; not active because the scheduled-task limit is reached | Build server-side import, RLS review queue and publication gate when workspace can edit |
...
2. The planned weekly connected-app review would record the affected function, last known successful check, evidence link, owner and next action in the Notion task register. It is not currently scheduled: the account's five active task slots are full. Never silently mark unavailable app checks as passing.
