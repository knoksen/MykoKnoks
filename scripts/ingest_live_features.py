    ...
    async def fetch(cell: str) -> dict:
        ...
        async with semaphore:
            snapshot = await service.probe(
                lat,
                lon,
                include_wms=not args.fast,
                include_terrain_metrics=not args.fast,
            )
        return {
            "h3": cell,
    ...
    with output.open("w", encoding="utf-8") as handle:
        for start in range(0, len(cells), args.batch_size):
            batch = await asyncio.gather(
                *(fetch(cell) for cell in cells[start : start + args.batch_size])
            )
            for row in batch:
...
def main() -> None:
    ...
    parser.add_argument(
        "--fast",
        action="store_true",
        help="Skip WMS and gradient probes; centre elevation/terrain only",
    )
